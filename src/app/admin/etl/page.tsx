...
export default async function AdminEtlPage() {
  ...
  let stats: Array<{ table_name: string; row_count: number }> = [];
  try {
    ...
    `);
    stats = result as unknown as typeof stats;
  } catch {
    // DB nedostupná
  }
          ...
          <ul className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {stats.map((s) => (
              <li key={s.table_name} className="p-3 rounded border border-border bg-card">
                ...
                <div className="text-2xl font-bold">{s.row_count.toLocaleString("cs-CZ")}</div>
              </li>
            ))}
          </ul>
        )}
